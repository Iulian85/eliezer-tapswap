...
const App: React.FC = () => {
  ...
  const handleSaveGame = () => {
    ...
    const gameData = {
      board, score, moves, timeLeft, levelIndex: playMode === 'CAMPAIGN' ? currentLevelIndex : -1,
      goalProgress, timestamp: Date.now(), inventory, stats: userStats, lastDailyCompleted
    };
    saveGame(gameData);
    setHasSave(true);
    if (telegramId) api.saveGame(telegramId, gameData);
  ...
  const handleRedeemReferral = (code: string) => {
      ...
      if (code.length >= 5) {
          ...
          setInventory(prev => ({
              ...
              boosters: { ...prev.boosters, bomb: prev.boosters.bomb + bonusBomb }
          }));
    ...
    if (c1 && c2 && (isSpecial1 || isSpecial2)) {
        ...
        if (comboResult.triggered) {
            ...
            setTimeout(() => {
                 ...
                 comboResult.clearedIndices.forEach(idx => clearBoard[idx] = null);
  ...
  const handleBuyBooster = async (item: 'bomb' | 'extraMoves' | 'shuffle', cost: number) => {
    ...
    const transaction = {
        ...
        messages: [{ address: TREASURY_WALLET, amount: toNano(cost) }],
    };
    try {
        ...
        setUserStats(prev => {
            ...
            const newStats = {
                ...prev,
                tonPurchases: prev.tonPurchases + cost, 
                purchaseHistory: [newRecord, ...prev.purchaseHistory]
            };
            const saveData = {
                board, score, moves, timeLeft, levelIndex: currentLevelIndex, goalProgress, timestamp: Date.now(),
                inventory: newInventory, stats: newStats, lastDailyCompleted
            };
            saveGame(saveData);
            if (telegramId) api.saveGame(telegramId, saveData);
            return newStats;
  ...
  const handleUseInGameBooster = (type: 'extraMoves' | 'bomb') => {
      ...
      if (type === 'bomb') {
          if (activeBooster === 'BOMB') { setActiveBooster(null); return; }
  ...
  const handleOpenShop = () => {
      if (!wallet) { if (tonConnectUI) tonConnectUI.openModal(); showToast("Connect TON Wallet to access Shop!"); } else { setIsShopOpen(true); }
  };
      ...
      {isShuffling && (
        <div className="absolute inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
             <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-2xl font-bold text-white tracking-widest uppercase animate-pulse">Shuffling...</div>
             </div>
        </div>
      )}
             ...
             <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 custom-scrollbar flex flex-col gap-3">
                 <div className="bg-white/5 p-1 rounded-xl flex gap-1 shrink-0">
                    {(['EASY', 'MEDIUM', 'HARD'] as Difficulty[]).map((d) => (
                        <button key={d} onClick={() => setDifficulty(d)} className={`flex-1 py-2 text-[10px] sm:text-xs font-bold rounded-lg transition-all ${difficulty === d ? 'bg-game-accent text-white shadow-lg' : 'text-white/40 hover:bg-white/10 hover:text-white'}`}>{d}</button>
                    ))}
                 </div>
                 <button onClick={openDailyPreview} className="w-full p-3 rounded-2xl bg-gradient-to-r from-yellow-500 to-orange-500 font-bold shadow-lg shadow-orange-500/20 flex items-center justify-between hover:brightness-110 active:scale-95 transition-all shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-white/20 rounded-lg"><Calendar className="text-white" size={18} /></div>
                        <div className="text-left"><div className="text-[10px] uppercase opacity-80">Daily Challenge</div><div className="text-sm font-black">Play Today's Level</div></div>
                    </div>
                 ...
                 <div className="bg-game-panel rounded-2xl p-2 border border-white/5 space-y-1.5">
                     {LEVELS.map((lvl, idx) => {
                         ...
                         return (
                             <button key={lvl.levelNumber} onClick={() => !isLocked && openLevelPreview(idx)} className={`w-full p-3 rounded-xl flex items-center justify-between group transition-all ${isLocked ? 'bg-white/5 opacity-50' : 'bg-white/10 hover:bg-white/20 hover:scale-[1.01] active:scale-95'}`}>
                                <div className="flex items-center gap-3">
                                    <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm shadow-inner ${isLocked ? 'bg-white/10' : 'bg-gradient-to-br from-blue-400 to-blue-600 border border-white/20'}`}>{isLocked ? <Lock size={12} /> : idx + 1}</div>
                                    <div className="text-left"><div className="font-bold text-sm">Level {idx + 1}</div><div className="text-[10px] text-white/50">{lvl.goals.length} Goals</div></div>
                                </div>
          ...
          <div className="absolute inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 p-4">
              <div className="bg-[#2a1b3d] w-full max-w-sm rounded-[2rem] border border-white/10 p-6 shadow-2xl flex flex-col gap-6 relative">
                  ...
                  <div className="text-center space-y-1 mt-2">
                      ...
                      <h2 className="text-3xl font-black text-white">{previewData.mode === 'DAILY' ? 'Daily Challenge' : `Level ${previewData.index + 1}`}</h2>
                      <div className="flex flex-wrap justify-center gap-3 mt-4">
                          {previewData.levelConfig.goals.map((g, idx) => (
                              <div key={idx} className="flex flex-col items-center gap-1">
                                  <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">{g.type === 'SCORE' ? <Target size={20} className="text-yellow-400" /> : <CandyIcon color={g.targetColor || CandyColor.Multi} type={g.targetCandyType === 'STRIPED_ANY' ? CandyType.StripedHorizontal : g.targetCandyType} size={20} />}</div>
                                  <span className="text-[10px] text-white/60 font-medium max-w-[60px] leading-tight">{g.type === 'SCORE' ? g.target.toLocaleString() : `x${g.target}`}</span>
                              </div>
                          ))}
                      </div>
                  </div>
                  <div className="space-y-3">
                      <div className="flex items-center justify-between px-1"><h3 className="text-xs font-bold text-white/50 uppercase tracking-widest">Select Boosters</h3><div onClick={handleOpenShop} className="text-xs font-bold text-game-accent cursor-pointer hover:underline flex items-center gap-1"><Plus size={12} /> Get More</div></div>
                      <div className="grid grid-cols-2 gap-3">
                          <div onClick={() => inventory.boosters.bomb > 0 && setPreGameBoosters(p => ({ ...p, bomb: !p.bomb }))} className={`relative p-3 rounded-2xl border transition-all flex flex-col items-center gap-2 cursor-pointer group ${preGameBoosters.bomb ? 'bg-red-500/20 border-red-500 ring-1 ring-red-500' : 'bg-white/5 border-white/10 hover:bg-white/10'} ${inventory.boosters.bomb === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}>
                              <div className="absolute top-2 right-2 bg-black/40 px-1.5 py-0.5 rounded-md text-[10px] font-bold text-white border border-white/10">x{inventory.boosters.bomb}</div>
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${preGameBoosters.bomb ? 'bg-red-500 shadow-red-500/50' : 'bg-white/10'}`}><Target size={20} className="text-white" /></div>
                              <div className="text-xs font-bold text-white">Bomb Candy</div>
                          </div>
                          <div onClick={() => inventory.boosters.extraMoves > 0 && setPreGameBoosters(p => ({ ...p, extraMoves: !p.extraMoves }))} className={`relative p-3 rounded-2xl border transition-all flex flex-col items-center gap-2 cursor-pointer group ${preGameBoosters.extraMoves ? 'bg-blue-500/20 border-blue-500 ring-1 ring-blue-500' : 'bg-white/5 border-white/10 hover:bg-white/10'} ${inventory.boosters.extraMoves === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}>
                               <div className="absolute top-2 right-2 bg-black/40 px-1.5 py-0.5 rounded-md text-[10px] font-bold text-white border border-white/10">x{inventory.boosters.extraMoves}</div>
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${preGameBoosters.extraMoves ? 'bg-blue-500 shadow-blue-500/50' : 'bg-white/10'}`}><Zap size={20} className="text-white fill-white" /></div>
          ...
          <>
            ...
            <div className="px-4 pb-2 flex items-end justify-between z-10">
                <div className="flex flex-col items-center">
                    <div className={`bg-game-panel px-4 py-1.5 rounded-full border border-white/10 flex items-center gap-2 shadow-lg transition-all duration-300 ${triggerMovesAnim ? 'scale-110 bg-yellow-500/20 text-yellow-300 ring-2 ring-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.6)]' : ''}`}><Move size={16} className={triggerMovesAnim ? 'text-yellow-300' : 'text-blue-300'} /><span className="font-black text-xl">{moves}</span></div>
                        ...
                        return (
                            <div key={goal.id} className={`flex flex-col items-center transition-all duration-500 ${isMet ? 'opacity-50 scale-90 grayscale' : ''}`}>
                                <div className="w-12 h-12 rounded-2xl bg-game-panel border border-white/10 flex items-center justify-center relative overflow-hidden">
                                    {goal.type === 'SCORE' ? <Target size={20} className="text-yellow-400" /> : <CandyIcon color={goal.targetColor || CandyColor.Multi} type={goal.targetCandyType === 'STRIPED_ANY' ? CandyType.StripedHorizontal : goal.targetCandyType} size={20} />}
                                    <div className="absolute bottom-0 left-0 right-0 bg-green-500/50 transition-all duration-500" style={{ height: `${(current / goal.target) * 100}%` }} />
                                ...
                                <span className="text-[10px] font-bold mt-1 text-white shadow-black drop-shadow-md">{current}/{goal.target}</span>
                            </div>
                        );
                    })}
                </div>
                <div className="flex flex-col items-center">
                    <div className={`bg-game-panel px-4 py-1.5 rounded-full border border-white/10 flex items-center gap-2 shadow-lg ${timeLeft < 10 ? 'animate-pulse text-red-400 border-red-400/50' : ''}`}><Clock size={16} className={timeLeft < 10 ? 'text-red-400' : 'text-purple-300'} /><span className="font-black text-xl font-mono">{Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</span></div>
            ...
            <div className="px-6 py-2 relative z-10">
                 <div className="w-full h-3 bg-black/40 rounded-full overflow-hidden border border-white/5">
                     <div className="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-orange-500 transition-all duration-500 ease-out relative" style={{ width: `${Math.min(100, (score / (activeLevel.goals.find(g => g.type === 'SCORE')?.target || 1000)) * 100)}%` }}><div className="absolute right-0 top-0 bottom-0 w-2 bg-white/50 blur-[2px]" /></div>
                 </div>
            ...
            <div className="flex-1 flex items-center justify-center p-4 relative z-0">
                <GameBoard board={board} selectedCandyIndex={selectedCandy} onTap={handleTap} onSwipe={handleSwipe} isProcessing={isProcessing} isShaking={isShaking} activeEffects={activeEffects} bombEffectIndex={bombEffectIndex} collectingIndices={collectingIndices} />
            </div>
            <div className="px-4 pb-6 pt-2 flex items-center justify-between gap-4 z-10 bg-gradient-to-t from-game-bg via-game-bg to-transparent">
                ...
                <div className="flex-1 bg-black/30 backdrop-blur-md rounded-2xl p-2 border border-white/10 flex justify-around shadow-xl">
                    <BoosterButton icon={Target} count={inventory.boosters.bomb} onClick={() => handleUseInGameBooster('bomb')} isActive={activeBooster === 'BOMB'} color="red" />
                    <BoosterButton icon={Zap} count={inventory.boosters.extraMoves} onClick={() => handleUseInGameBooster('extraMoves')} color="blue" />
                    <BoosterButton icon={Shuffle} onClick={handleManualShuffle} color="green" disabled={!shuffleAvailable && inventory.boosters.shuffle === 0} label={shuffleAvailable ? "FREE" : undefined} count={!shuffleAvailable ? inventory.boosters.shuffle : undefined} />
                </div>
      ...
      {isCelebrating && (
        <div className="absolute inset-0 z-[90] flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-500 pointer-events-none">
            ...
            <div className="animate-bounce-in flex flex-col items-center p-8 rounded-3xl bg-gradient-to-b from-white/10 to-transparent border border-white/20 backdrop-blur-md shadow-2xl">
                <div className="w-24 h-24 bg-yellow-400 rounded-full flex items-center justify-center shadow-[0_0_40px_rgba(250,204,21,0.6)] mb-4 animate-pulse"><Trophy size={48} className="text-yellow-900" /></div>
                <h2 className="text-4xl font-black text-white drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)] tracking-wider text-center">LEVEL<br/>COMPLETE!</h2>
                <div className="mt-2 text-yellow-300 font-bold text-xl drop-shadow-md">Score: {score.toLocaleString()}</div>
            </div>
        </div>
      )}
      {gameState === 'LOST' && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-[#2a1b3d] border border-white/10 p-8 rounded-[2rem] shadow-2xl text-center max-w-xs w-full">
                <div className="w-20 h-20 bg-white/10 rounded-full mx-auto flex items-center justify-center mb-4"><RotateCcw size={40} className="text-white/50" /></div>
                ...
                <div className="space-y-3">
                    <button onClick={() => showAd(() => initGame(activeLevel))} className="w-full py-3.5 rounded-xl bg-white text-black font-bold hover:bg-gray-100 transition-colors">Try Again</button>
                    <button onClick={() => setGameState('INTRO')} className="w-full py-3.5 rounded-xl bg-white/5 text-white font-bold hover:bg-white/10 transition-colors flex items-center justify-center gap-2"><Home size={16} /> Main Menu</button>
                </div>
            </div>
        </div>
      )}
...
const BoosterButton: React.FC<{ icon: React.FC<any>; count?: number; onClick: () => void; isActive?: boolean; color: 'red' | 'blue' | 'green'; label?: string; disabled?: boolean; }> = ({ icon: Icon, count, onClick, isActive, color, label, disabled }) => {
    ...
    return (
        <button onClick={onClick} disabled={disabled} className={`relative w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-200 ${isActive ? 'scale-110 ring-2 ring-white ring-offset-2 ring-offset-black' : 'hover:scale-105 active:scale-95'} ${(!disabled && (count === undefined || count > 0 || label === 'FREE')) ? 'bg-white/10 text-white' : 'bg-white/5 text-white/20 cursor-not-allowed'} ${isUsed ? 'bg-white/20 ring-1 ring-white/50' : ''} ${disabled ? 'grayscale opacity-50' : ''}`}>
            <Icon size={20} className={`${isActive ? 'animate-pulse' : ''} ${isUsed ? 'scale-125 duration-150' : ''}`} />
            {(count !== undefined && count > 0) && (<div className={`absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold border border-white/20 shadow-lg transition-all duration-300 ${colors[color]} text-white ${isUsed ? 'scale-150 bg-yellow-400 text-black -translate-y-2' : 'scale-100'}`}>{count}</div>)}
            {label && (<div className={`absolute -top-2 -right-2 px-1.5 py-0.5 rounded-md flex items-center justify-center text-[9px] font-bold border border-white/20 shadow-lg transition-all duration-300 ${disabled ? 'bg-gray-600' : colors[color]} text-white`}>{label}</div>)}
            {isUsed && (<div className="absolute inset-0 rounded-xl bg-white/30 animate-ping" />)}

...
import { generateSecureReferralCode, decodeReferralCode, safeSendReferralNotification } from './src/utils/referralUtils';
...
const App: React.FC = () => {
  ...
  const handleClaimDailyReward = () => {
    ...
    showToast(`Daily Login: +${rewardCoins} Coins & 1 Shuffle!`);
  };
          ...
          <div className="flex-1 flex flex-col h-full animate-in fade-in duration-500 relative">
             ...
             <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 custom-scrollbar flex flex-col gap-3">
                 ...
                 <button 
                    ...
                    disabled={userStats.lastLoginRewardDate === getTodayDateString()}
                    className={`w-full p-3 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-600 font-bold shadow-lg shadow-rose-500/20 flex items-center justify-between transition-all shrink-0 border border-white/10
                        ${userStats.lastLoginRewardDate === getTodayDateString() ? 'opacity-80' : 'hover:brightness-110 active:scale-95 animate-pulse'}`}
                 >
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-white/20 rounded-lg"><Gift className="text-white" size={18} /></div>
                        <div className="text-left">
                            <div className="text-[10px] uppercase opacity-80 font-bold">Daily Reward</div>
                            <div className="text-sm font-black">{userStats.lastLoginRewardDate === getTodayDateString() ? 'Claimed Today' : 'Tap to Claim Gift!'}</div>
                        </div>
                    </div>
                    {userStats.lastLoginRewardDate === getTodayDateString() ? <CheckCircle className="text-white/80" /> : <div className="bg-white text-rose-600 rounded-full p-1"><Plus size={12} strokeWidth={4} /></div>}
                 </button>
